...
export function useSidebarItemsToHighlight(): [string | undefined, string | undefined] {
  ...
  const highlightCategory = SiderMenuItems.find((it) => {
    return router.pathname.startsWith(it.url);
  });
  ...
  const highlightSubcategoryUrl = highlightCategory?.children?.find((it) => {
    return router.pathname.startsWith(it.url);
  })?.url;
